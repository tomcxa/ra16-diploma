...
const Catalog = ({ children }) => {
  ...
  return (
    <section className="catalog">
      ...
      {/* {(loading && !offset) && <Loader />} */}
      {children}
      <CatalogCategories categoryChange={categoryChange} />
      <CatalogCardList data={cards} loading={loading} error={error} />
      {data.length < 6 ? null : <div className="text-center">
        <button className="btn btn-outline-primary" onClick={handleLoad} disabled={loading} >Загрузить ещё</button>
      </div>}
    </section>
  );
};
